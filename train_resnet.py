...
def create_model(depth=101, input_size=321, num_classes=21, name=None):
    ...
    inputs = {
        ...
        'label': 'uint32({}, {}, 1, _)'.format(input_size, input_size),
        'label_weight': 'float32({}, {}, 1, _)'.format(input_size, input_size)
    }
    ...
    # resize label and label_weight
    label = main.var('label')
    label_weight = main.var('label_weight')
    shrink_label = label.to(Interp_shrink(shrink_factor=8, interp_type='nearest_neighbor'), name='shrink_label')
    shrink_label_weight = label_weight.to(Interp_shrink(shrink_factor=8, interp_type='nearest_neighbor'), name='shrink_label_weight')
    ...
    x_fusion = main.vars(x, x_05_zoom, x_075_zoom).to(Maxout(), name='fc_fusion')

    # label and predict are both 1/8 original size, used for cal loss
    main.vars(x, 'shrink_label', 'shrink_label_weight').to(SoftmaxWithLoss(axis=2), name='loss_res1')
    main.vars(x_05_zoom, 'shrink_label', 'shrink_label_weight').to(SoftmaxWithLoss(axis=2), name='loss_res05')
    main.vars(x_075_zoom, 'shrink_label', 'shrink_label_weight').to(SoftmaxWithLoss(axis=2), name='loss_res075')
    main.vars(x_fusion, 'shrink_label', 'shrink_label_weight').to(SoftmaxWithLoss(axis=2), name='loss_fusion')
    model = main.compile(inputs=inputs, seal=False)
    model.add_flow('main', inputs.keys(), ['loss_res1', 'loss_res05', 'loss_res075', 'loss_fusion', 'accuracy_top1', 'accuracy_top5'], ['loss_res1', 'loss_res05', 'loss_res075', 'loss_fusion'])
...
if __name__ == '__main__':
    train_model = create_model(101)
    with open('train.yaml', 'w+') as train_file:
        train_file.write(train_model.to_yaml_text())
