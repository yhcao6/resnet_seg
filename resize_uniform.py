...
import numpy as np
import os
def resize_pad_to_fit(img, size, pad_value=0, interp=cv2.INTER_LINEAR): # size: [rows, cols], to uniform size
    if len(size) != 2:
    ...
    out_img = pad_value * np.ones(tuple(size + [channels]), img.dtype)
